...
namespace SeasonalBastion.WorldGen.Runtime.Meshes
{
    public static class TerrainMeshGenerator
    {
        public static MeshData GenerateTerrainMesh(float[,] heightMap, WorldMeshSettings meshSettings, int levelOfDetail)
        {
            int skipIncrement = levelOfDetail == 0 ? 1 : levelOfDetail * 2;
            int numVertsPerLine = meshSettings.NumVertsPerLine;

            Vector2 topLeft = new Vector2(-1f, 1f) * meshSettings.MeshWorldSize / 2f;
            ...
            for (int y = 0; y < numVertsPerLine; y++)
            {
                for (int x = 0; x < numVertsPerLine; x++)
                {
                    ...
                    if (isEdgeConnectionVertex)
                    {
                        ...
                        height = heightMainVertexA * (1f - dstPercentFromAToB) + heightMainVertexB * dstPercentFromAToB;
                    }
